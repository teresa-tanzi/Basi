<!--connessione ad DB per aggiungere cinguettii di tipo testo-->
...
<?php
	...
	$mex="";
	if ($_POST["testo"]=="") {
		$mex=$mex."Non è stato inserito alcun testo<br/>";
	} elseif (strlen($_POST["testo"])>100) {
			$mex=$mex."Il testo è troppo lungo<br/>";
		} else {
			$testo=$_POST["testo"];
	}
	...
	//inserisco i dati nella tabella 'cinguettio', dovrò poi copiare utente ed id in quella specializzata (qui testo)
	...
	//inserisco i dati nella tabella 'testo'
	$query_t="INSERT INTO testo
		VALUES ('$email', $id, '$testo')";
	$query_t_res=pg_query($con, $query_t);
	if (!$query_t_res) {
		echo "Errore: ".pg_last_error($con);
		exit;
	}
